...
static recursive_mutex *rm;

/*
 * <FIRST TEST>
 */
static void
delete_index(int **array, int index){
...
static void
delete_array(int **array, int size){
    int i;

    recursive_mutex_lock(rm);
    for (i = 0; i < size; i++){
	delete_index(array, i);
    }
    recursive_mutex_unlock(rm);
}

static void
check_array(int **array, int size){
    int i;
